...
void update_item_hits(item *it);
void update_item_miss(char *key);
void update_item_set(item *it);
void update_item_delete(item *it);
...
static void proccess_stat_pdf(ADD_STAT add_stats, void *c) {
    assert(add_stats);
    int i, clsid;
    char buf[128];
    for (clsid = 0; clsid <= get_power_largest(); clsid++) {
      for (i = 0; i <100; i++) {
         snprintf(buf, 128, "pdf_%d_%d", clsid, i);
...
static void process_stat(conn *c, token_t *tokens, const size_t ntokens) {
    ...
    if (ntokens == 2) {
        ...
        return ;
    } else if (strncmp(subcommand, "cdf", 3) == 0) {
        // send the CDF array as a reponse
        proccess_stat_cdf(&append_stats, c);
    } else if (strncmp(subcommand, "pluscdf", 7) == 0) {
        // send the CDF array as a reponse
        proccess_stat_pluscdf(&append_stats, c);
    } else if (strncmp(subcommand, "fastplus", 8) == 0) {
    ...
    } else if (strncmp(subcommand, "pdf", 3) == 0) {
        // send the PDF array as a reponse
        proccess_stat_pdf(&append_stats, c);
        ...
        while(key_token->length != 0) {
            ...
            if (it) {
                ...
                *(c->ilist + i) = it;
                ...
                pthread_mutex_unlock(&c->thread->stats.mutex);
                MEMCACHED_COMMAND_GET(c->sfd, key, nkey, -1, 0);
                update_item_miss(key); // this is called from the main thread
            }
...
int main (int argc, char **argv) {
    ...
    while (-1 != (c = getopt(argc, argv,
        ...
        switch (c) {
        case 'q':
            statistics_buckets = atoi(optarg);
            break;
        case 'U':
    ...
    pdf = calloc(POWER_LARGEST, sizeof( int *));
    int clsid;
    for (clsid = 0; clsid < POWER_LARGEST; clsid++) {
      pdf[clsid] = calloc(100,  sizeof(int));
    }
    statistics_init(statistics_buckets); // get value from a parameter ?
