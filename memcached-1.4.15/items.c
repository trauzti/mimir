...
{
#ifdef ACCURATE_PDF
  int j=get_true_stackdistance(it);
  int bucket = (int) ( 100.0 * (double)  j  / (double) sizes[it->slabs_clsid]);
  pdf[it->slabs_clsid][bucket]++;
  //update_requests(j);
  //update_requests(0);
#else
...
void update_item_miss(char *key)
{
  ...
  // do nothing
#else
  // TODO: we need the real key for the ghostlist to work
  // TODO: get the correct slab class
...
void update_item_delete(item *it)
{
 ...
 // do nothing
#else
  statistics_evict(it->slabs_clsid, it);
#endif
  return;
}
...
int get_true_stackdistance(item *it)
{
  ...
  item *search;
  int j=0;
  //frpintf(stderr, "head and down: ");
  search = heads[it->slabs_clsid];
  for(;search!=it && search !=0;search=search->next)
  {
    if (search->next)
      assert ( search->activity >= search->next->activity);
    //fprintf(stderr, "%u, ", search->activity);
    j++;
  }
  //fprintf(stderr, "\n");
  if (search ==0 ) {
    assert (j == sizes[it->slabs_clsid]);
    assert(0);
  }
  mutex_unlock(&cache_lock);
  return j;
