...
static void process_bin_get_or_touch(conn *c) {
    ...
    if (should_touch) {
        ...
        hv = hash(key, nkey);
        it = item_get_with_hash(key, nkey, hv);
    }
    ...
    if (it) {
        ...
        pthread_mutex_lock(&c->thread->stats.mutex);
        // the hash is calculated in item_get .. it would be a waste to recalculate it again here..
        // another get function with precomputed hash
        // wtf touch...
        if (should_touch) {
            c->thread->stats.touch_cmds++;
            ...
            statistics_miss(0, key, hv, nkey); // put the ghostlist in here and measure the throughput !!!
        }
        pthread_mutex_unlock(&c->thread->stats.mutex);
